...
export async function POST(request: NextRequest) {
  ...
  if (!match || match.state.status !== 'picking' || !match.state.letter) {
    return NextResponse.json({ error: 'اختاروا حرف الأول.' }, { status: 400 });
  }

  const { error } = await supabase
    .from('game_matches')
    .update({ state: { ...match.state, status: 'answering' }, updated_at: new Date().toISOString() })
    .eq('id', matchId);
